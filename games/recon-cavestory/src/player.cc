...
void Player::initializeSprite(Graphics& graphics, const SpriteState& sprite_state) {
    ...
    // source_y will be characterFrame*kTileSize if we are going left
    // source_y will be (characterFrame+1)*Game::kTileSize otherwise
    ...
    source_x = sprite_state.vertical_facing == UP ?
        source_x + kUpFrameOffset*Game::kTileSize : source_x;
...
void Player::initializeSprites(Graphics& graphics) {
    //For every MotionType
        //For every HorizontalFacing
            //For every VerticalFacing
