package com.shadowh511.mayor.util.circleFinder;
...
public class Backend {
    ...
    public static Backend[] findCircularTargets(ColorImage image) throws NIVisionException { //include gregs other parameters?
        ...
        SortedVector.Comparator targetComparator = new SortedVector.Comparator() {
            ...
            public int compare(Object target1, Object target2) {
                if (((Backend) target1).m_score > ((Backend) target2).m_score) {
                    return 1;
                }
                if (((Backend) target1).m_score < ((Backend) target2).m_score) {
                    return -1;
        ...
        while (!sortedTargetList.isEmpty()) {
            Backend firstTarget = (Backend) sortedTargetList.elementAt(0);
            for (int i = 1; i < sortedTargetList.size(); i++) {
                Backend otherTarget = (Backend) sortedTargetList.elementAt(i);
        ...
        Backend[] toReturn = new Backend[combinedTargetList.size()];
